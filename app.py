...
def create_playlist(payload: dict = Body(...)):
    """Builds a 30-track mood pool, saves it as a private Spotify playlist in
    the user's account, and returns the open.spotify.com URL. Used by the
    mobile flow to hand off to the Spotify app (where full tracks play)."""
    user_sp = get_user_sp()
    if not user_sp:
    ...
    pool = pool[:30]
    uris = [f"spotify:track:{t['id']}" for t in pool if t.get("id")]
    if not uris:
        raise HTTPException(status_code=500, detail="Pool produced no playable tracks")

    try:
        user_id = user_sp.me()["id"]
        playlist = user_sp.user_playlist_create(
            user_id,
            name=f"Mood DJ · {mood[:80]}",
            public=False,
            description=f"Generated by Mood DJ for the mood: {mood[:200]}",
        )
        user_sp.playlist_add_items(playlist["id"], uris)
        return {
